...
HALV_paintshop_opendialog = {
	...
	{
		["ErrorTitleOnly", ["You cannot paint locked vehicles"]] call ExileClient_gui_toaster_addTemplateToast;
	};
...
while{alive player}do{
	_nearpaintshops = (nearestObjects [player,_paintshopbuildings,150])-[player];
		...
		if(player isEqualTo vehicle player)then{
			if !(Backpack player in ["","B_Parachute","B_O_Parachute_02_F","B_I_Parachute_02_F","B_B_Parachute_02_F"])then{
				if(_HALV_panitshop_bagaction< 0)then{
					//_HALV_panitshop_bagaction = player addAction ["<t>Paint Backpack</t>", {(_this select 3) call HALV_paintshop_opendialog;}, (unitBackpack player),1, false, true, "", ""];
				};
			}else{
			...
			if !(Uniform player in ["","U_Test1_uniform","U_Test_uniform"])then{
				if(_HALV_panitshop_uniformaction< 0)then{
					//_HALV_panitshop_uniformaction = player addAction ["<t>Paint Uniform</t>", {(_this select 3) call HALV_paintshop_opendialog;}, player,1, false, true, "", ""];
				};
			}else{
				player removeAction _HALV_panitshop_uniformaction;
				_HALV_panitshop_uniformaction = -1;
			};
			_nearvehicles = (player nearEntities [["Air","LandVehicle","Ship"],10])-[player];
			...
			if(count _HALV_panitshop_vehicleactions < 1)then{
				{
					if !(_x isKindOf "Wreck_Base")then{
						...
						_action = player addAction [format["<t>Paint %1</t>",_txt], {(_this select 3) call HALV_paintshop_opendialog;}, _x,1, false, true, "", ""];
